#!/usr/bin/env python3
"""
WiFi Barcode Scanner — Desktop Companion
...
# ── paths & config ────────────────────────────────────────────────────────────
APP_DIR   = Path.home() / ".barcode_companion"
...
def make_tray_image(active: bool = True) -> Image.Image:
    ...
    bg   = (30, 35, 42, 255)
    fg   = (57, 255, 20, 255) if active else (100, 100, 100, 255)

    # Rounded rect background
    d.rounded_rectangle([0, 0, size - 1, size - 1], radius=10, fill=bg)

    # Barcode lines (simple representation)
    bars = [6, 9, 12, 15, 20, 23, 27, 30, 34, 37, 41, 44, 48, 51, 55, 58]
    for i, x in enumerate(bars):
        w = 2 if i % 3 != 0 else 3
        d.rectangle([x, 10, x + w, 42], fill=fg)

    # WiFi arc (bottom)
    d.arc([22, 46, 42, 58], start=200, end=340, fill=fg, width=2)
    d.ellipse([30, 53, 34, 57], fill=fg)
...
class ScanHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/ping":
            self._json(200, {"status": "ok", "server": "WiFi Barcode Companion"})
        else:
...
DARK  = {"bg": "#1a1f27", "fg": "#e8edf5", "acc": "#39ff14",
          "card": "#242b36", "border": "#2e3844", "entry_bg": "#0f1318",
          "dim": "#7a8599", "red": "#ff4757", "yellow": "#ffa502"}
LIGHT = {"bg": "#f0f4f8", "fg": "#1a1f27", "acc": "#1a8a00",
          "card": "#ffffff", "border": "#d1dbe8", "entry_bg": "#ffffff",
          "dim": "#5a6a7e", "red": "#cc2200", "yellow": "#cc7700"}
...
class SettingsWindow:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("WiFi Barcode Companion")
    ...
    def _apply_theme(self):
        ...
        style.configure("TButton",      background=t["acc"],  foreground="#000000",
                        font=("Consolas", 10, "bold"), padding=6, relief="flat")
        style.map("TButton",
                  background=[("active", "#55ff2a"), ("disabled", t["border"])])
        style.configure("Stop.TButton", background=t["red"],  foreground="#ffffff")
        ...
        style.configure("TCombobox",    fieldbackground=t["entry_bg"],
                        foreground=t["fg"], font=("Consolas", 10))
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="⬡  WiFi Barcode Companion",
                 bg=t["bg"], fg=t["acc"],
    ...
    def _build_dashboard(self, parent):
        ...
        tk.Label(scard, textvariable=self._status_var,
                 bg=t["card"], fg=t["fg"],
                 font=("Consolas", 11)).pack(anchor="w", pady=(6, 0))
        ...
        tk.Label(lcard, textvariable=self._last_var,
                 bg=t["card"], fg=t["acc"],
                 font=("Consolas", 14, "bold")).pack(anchor="w", pady=(4, 0))
    ...
    def _stat_card(self, parent, label, value):
        ...
        tk.Label(fr, textvariable=var, bg=t["card"], fg=t["fg"],
    ...
    def _build_server(self, parent):
        ...
        tk.Label(parent, text="HTTP Listener Configuration",
                 bg=t["bg"], fg=t["acc"],
                 font=("Consolas", 12, "bold")).pack(anchor="w", **pad)
        ...
        self._chk_notify = self._checkbox(parent, "Show desktop notifications",
                                           "show_notify", pad)
        self._chk_log    = self._checkbox(parent, "Log scans to file  (~/.barcode_companion/scans.log)",
                                           "log_scans", pad)
        ...
        for v, lbl in [("dark", "Dark"), ("light", "Light")]:
            tk.Radiobutton(row2, text=lbl, variable=self._theme_var, value=v,
                           ...
                           activebackground=t["bg"], font=("Consolas", 10)).pack(side="left", padx=8)
        ...
        tk.Label(parent, text="API Endpoints", bg=t["bg"], fg=t["acc"],
                 font=("Consolas", 11, "bold")).pack(anchor="w", padx=18)
    ...
    def _build_keyboard(self, parent):
        ...
        tk.Label(parent,
                 ...
                 bg=t["bg"], fg=t["dim"],
                 font=("Consolas", 9)).pack(anchor="w", padx=18, pady=4)
