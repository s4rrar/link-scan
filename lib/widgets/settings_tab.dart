...
class _SettingsTabState extends State<SettingsTab> {
  ...
  void _onIpChanged() {
    ...
    if (ip.isNotEmpty && ip != widget.viewModel.serverIp) {
  ...
  void _onPortChanged() {
    ...
    if (port != null && port >= 1 && port <= 65535 && port.toString() != widget.viewModel.serverPort.toString()) {
      widget.viewModel.updateServerPort(port);
    }
  }

  void _onViewModelChanged() {
    if (_ipController.text != widget.viewModel.serverIp) {
      _ipController.text = widget.viewModel.serverIp;
    }
    final portStr = widget.viewModel.serverPort.toString();
    if (_portController.text != portStr) {
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      style: OutlinedButton.styleFrom(
                        ...
                        side: BorderSide(color: polishPrimary.withOpacity(0.5), width: 1.5),
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppStyles.radiusMedium)),
                      ),
                      onPressed: viewModel.isScanningUdp || viewModel.isTestingConnection
                          ? null
                          : () => viewModel.scanForServers(),
                      icon: viewModel.isScanningUdp
                          ? SizedBox(
                              width: 20.0,
                              height: 20.0,
                              child: CircularProgressIndicator(color: polishPrimary, strokeWidth: 2.0),
                            )
                          : const Icon(Icons.radar_outlined, size: 18.0),
                      label: Text(
                        viewModel.isScanningUdp ? 'Scanning Local Network...' : 'Autodetect PC Server',
                        style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14.0),
                      ),
                    ),
                  ),

                  // If multiple servers detected, show dropdown selector
                  if (viewModel.discoveredServers.length > 1) ...[
                    const SizedBox(height: AppStyles.spacingNormal),
                    Text(
                    ...
                    const SizedBox(height: AppStyles.spacingSmall),
                    DropdownButtonFormField<DiscoveredServer>(
                      value: viewModel.discoveredServers.any((s) => s.ip == viewModel.serverIp && s.port == viewModel.serverPort)
                          ? viewModel.discoveredServers.firstWhere((s) => s.ip == viewModel.serverIp && s.port == viewModel.serverPort)
                          : null,
                      decoration: InputDecoration(
                        focusedBorder: OutlineInputBorder(
                          borderSide: BorderSide(color: polishPrimary, width: 1.5),
                      ...
                      style: TextStyle(color: polishOnSurface, fontSize: 14.0),
                      hint: Text('Select server...', style: TextStyle(color: polishOnSurfaceVariant)),
                      items: viewModel.discoveredServers.map((server) {
                        return DropdownMenuItem<DiscoveredServer>(
                          value: server,
                          child: Text('${server.hostname} (${server.ip}:${server.port})'),
                        );
                      }).toList(),
                      onChanged: (selectedServer) {
                        if (selectedServer != null) {
                          viewModel.updateServerIp(selectedServer.ip);
                          viewModel.updateServerPort(selectedServer.port);
                        }
                      },
                    ),
                  ],
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      onPressed: viewModel.isTestingConnection || viewModel.isScanningUdp
                          ? null
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Icon(
                                viewModel.connectionStatusMessage!.contains('Connected') || viewModel.connectionStatusMessage!.contains('Detected')
                                    ? Icons.check_circle
                                    : Icons.error,
                                color: viewModel.connectionStatusMessage!.contains('Connected') || viewModel.connectionStatusMessage!.contains('Detected')
